...
def process_sensor_sample(params: Params, machine: TamperStateMachine, sensor_timestamp_ns: int,
  ...
  if event is not None:
    params.put("TamperModeLastEvent", event_to_param(event), block=True)
  return status, event
...
def main() -> None:
  ...
  try:
    ...
    while True:
      ...
      last_status, _ = process_sensor_sample(params, machine, sensor.timestamp, sensor.acceleration.v, last_status)
  finally:
    clear_capture_deadline(params)
